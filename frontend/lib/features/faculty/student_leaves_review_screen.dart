...
class StudentLeavesReviewScreen extends StatefulWidget {
  const StudentLeavesReviewScreen({super.key});

  @override
  State<StudentLeavesReviewScreen> createState() => _StudentLeavesReviewScreenState();
}

class _StudentLeavesReviewScreenState extends State<StudentLeavesReviewScreen> {
  List<dynamic> _list = [];
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    try {
      final data = await ApiService.getStudentLeavesForFaculty();
      if (mounted) setState(() {
        _list = data;
        _loading = false;
      });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Student Leave Applications')),
      body: _loading ? const Center(child: CircularProgressIndicator()) : _list.isEmpty ? const EmptyStateMessage(message: 'No student leave applications to review.', icon: Icons.event_busy) : ListView.builder(
